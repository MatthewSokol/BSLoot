...
local moduleName = addonName.."_window_bulk_gp"
local bsloot_window_bulk_gp = bsloot:NewModule(moduleName)
...
function bsloot_window_bulk_gp:OnEnable()
  ...
  editBox:SetCallback("OnEnterPressed",function()
    bsloot:importGpValues(self.editBox:GetText(), self.overwriteFlag:GetValue())
    bsloot_window_bulk_gp:Hide()
  end)
...
function bsloot_window_bulk_gp:Show()
...
function bsloot_window_bulk_gp:Hide()
  if self._big_container.frame:IsShown() then
    self._big_container:Hide()
  end
end
function bsloot_window_bulk_gp:Toggle()
  if self._big_container.frame:IsShown() then
    self._big_container:Hide()
  else
    bsloot_window_bulk_gp:Show()
  end
end
